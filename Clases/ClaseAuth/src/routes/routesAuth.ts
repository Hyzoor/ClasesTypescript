...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        if (exists) return res.status(404).json({ message: "Existe un usuario con el email" });

        // Omito validar email y passwd

        const psswdEncrypted = await bcrypt.hash(password, 10);
        await coleccion().insertOne({ email: email, password: psswdEncrypted });
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.status(404).json({ message: "El email no corresponde a ningun usuario" });
        ...
        res.status(200).json({ message: "Login correcto", token: "Bearer " + token })
